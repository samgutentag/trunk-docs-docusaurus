...
Trunk Flaky Tests integrates with your CI by adding a step in your GitHub Action workflow to upload tests with the [Trunk Uploader CLI](../../uploader).
...
Add an `Upload Test Results` step after running tests in each of your CI jobs that run tests. This should minimally include all jobs that run on pull requests, as well as jobs that run on your main or [stable branches](../../detection#stable-branches), for example, `main`, `master`, or `develop`.
...
<Tabs>
<TabItem value="junit-xml" label="JUnit XML">
...
<TabItem value="xcresult-path" label="XCResult Path">
...
<TabItem value="bazel-bep-json" label="Bazel BEP JSON">
...
<TabItem value="rspec-plugin" label="RSpec plugin">
