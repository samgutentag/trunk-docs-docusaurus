...
You can learn more about how tests are uploaded to Trunk before they're labeled in the [Get Started docs](get-started/). You can learn more about how detection results are displayed in the [Dashboard docs](dashboard).
...
<ThemedImage
  alt="The uploads tab contains results received from past CI jobs."
  sources={{
...
Expect test results for individual PRs to be up-to-date for [PR Test Summaries](github-pull-request-comments) within 15 minutes post-upload and all other metrics to be up-to-date within an hour.
...
You can specify a variant during upload using the [`--variant` option](uploader#full-command-reference):

``` title="Upload an iOS variant "
...
<ThemedImage
  alt="The same test, but the first is a macOS variant."
  sources={{
    light: "/img/variants-light-border.png",
    dark: "/img/variants-dark-border.png",
  }}
/>
...
<figure><img src="/img/manual-test-mark.png" alt="" /><figcaption></figcaption></figure>
...
If you're curious about why certain tests are labeled as flaky, you can visit each test's status history. [Learn more about Status History.](dashboard#status-history)
