...
You can install the [Trunk Launcher](cli/getting-started/install#the-trunk-launcher) script directly by downloading it through cURL. The launcher script supports both macOS and Linux environments.
...
Only some versions of Trunk are compatible with Windows. See the compatibility page for [Windows](cli/getting-started/compatibility) to learn more.
